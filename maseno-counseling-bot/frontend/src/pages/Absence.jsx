...
const Absence = () => {
  ...
  async function markAbsent(e) {
    ...
    console.log("Marking absence for date:", date);

    try {
      setLoading(true);
      const response = await api.post("/dashboard/absence", { date });
  ...
  async function removeAbsence(id) {
    ...
    console.log("Removing absence with ID:", id);

    try {
      const response = await api.delete(`/dashboard/absence/${id}`);
      console.log("Remove absence response:", response);
      loadDays();
      setError(null);
    } catch (err) {
